            ...
            <div class="card-body hover-scroll-overlay-y">
                <form class="form-data" enctype="multipart/form-data">
                    ...
                    <div class="mb-10">
                        ...
                        <select name="uuid_kategori" class="form-select" data-control="select2" id="from_select"
                    ...
                    <div class="mb-10">
                        <label for="deskripsi_soal" class="form-label">Deskripsi Soal <small class="fst-italic"
                                style="color: red">(bisa tidak
                                di isi)</small></label>
                        <textarea id="deskripsi_soal" name="deskripsi_soal" class="form-control" data-kt-autosize="true"></textarea>
                        <small class="text-danger deskripsi_soal_error"></small>
                    </div>

                    <div class="mb-10">
                        <label for="soal" class="form-label">Soal <small class="fst-italic" style="color: red">(bisa
                                tidak
                                di isi)</small></label>
                        <textarea id="soal" name="soal" class="form-control" data-kt-autosize="true"></textarea>
                        <small class="text-danger soal_error"></small>
                    </div>
                    ...
                    <div class="mb-10">
                        ...
                        <input type="text" id="jawaban_benar" class="form-control" name="jawaban_benar">
                        <small class="text-danger jawaban_benar_error"></small>
                    </div>
...
@section('script')
    <script>
        ...
        $(document).ready(function() {
            // Simpan HTML awal dari kontainer input
            ...
            $(document).on('click', '.button-update', function(e) {
                e.preventDefault();
                let url = '/admin/show-soal/' + $(this).attr('data-uuid');
                $(".title_side_form").html('Update Soal');
                ...
                $.ajax({
                    ...
                    success: function(res) {
                        if (res.success) {
                            // Ambil semua data jawaban
                            let jawaban = res.data.jawaban;

                            // Kosongkan elemen Container sebelum menambahkan data baru
                            $("#ContainerAwal").empty();

                            // Loop untuk menangani semua data jawaban
                            jawaban.forEach(function(data) {
                                var newInput = `
                                                    <div class="mb-10 inputGroup">
                                                        <label class="form-label">Jawaban</label>
                                                        <input type="text" class="form-control" name="jawaban[]" value="${data}">
                                                        <small class="text-danger jawaban_error"></small>
                                                        <div class="mt-10 d-grid">
                                                            <button type="button" class="btn btn-danger btn-sm hapusInputBtn">Hapus</button>
                                                        </div>
            ...
            // Fungsi untuk mengembalikan inputan ke keadaan semula
            $(document).on('click', '#button-side-form', function() {
                control.overlay_form('Tambah', 'Soal');
                // Kembalikan HTML awal ke kontainer input
        ...
        $(document).on('submit', ".form-data", function(e) {
            ...
            if (type == 'add') {
                control.submitFormMultipartData('/admin/add-soal', 'Tambah',
                    'Soal',
                    'POST');
            } else {
                let uuid = $("input[name='uuid']").val();
                control.submitFormMultipartData('/admin/update-soal/' + uuid,
                    'Update',
                    'Soal', 'POST');
            }
        });
        ...
        $(document).on('click', '.button-delete', function(e) {
            e.preventDefault();
            let url = '/admin/delete-soal/' + $(this).attr('data-uuid');
                ...
                columnDefs: [{
                    ...
                    render: function(data, type, full, meta) {
                        return `
                            ...
                            <a href="javascript:;" type="button" data-uuid="${data}" data-label="Soal" class="btn btn-danger button-delete btn-icon btn-sm">
                                <svg width="25" height="24" viewBox="0 0 25 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                        ...
                        if (!groupedData[group]) {
                            ...
                            return $("<tr/>")
                                .append(
                                    ...
                                    '<span style="font-weight: bold; color: #5caceb; padding: 2px 12px; font-size: 12px; border-radius: 8px; text-transform: uppercase;"> kategori soal = ' +
                                    group +
                                    '</span>' +
                                    '</td>'
        ...
        $(function() {
            initDatatable();
            control.push_select_kategori('/admin/get-kategori', '#from_select')
        });
    </script>
@endsection
